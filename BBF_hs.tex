...
dem Befehl \textit{\textasciitilde dirt.loadSoundFiles(''full/path/to/directory/*'')}. Mit diesem Befehl lässt sich ein Verzeichnis in Tidal integrieren. Anschließend können die Sounds beim Programmieren mit dem Ordnernamen benutzt werden.\cite{tidcy}
...
Art der Synthetisierung: Da Bass Drum und High Hat normal gespielt werden können die Sounds aus dem Supercollider 
mit minimaler Anpassung benutzt werden. Lediglich die Rim aus Figur 4 muss, wegen ihres hölzernen Sounds, selbst 
aufgenommen werden.\\
\noindent \textbf{Figur 1}\\
Sound BD: Die Base Drum wird mit zunehmender dauer lauter gespielt.\\
...
Lösung:
\begin{lstlisting}
p "i4" $ sound "~ bd hh ~" # cut 1 # midinote 15 #gain "1.3 0"
\end{lstlisting}
